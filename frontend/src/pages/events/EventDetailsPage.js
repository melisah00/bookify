...
const EventDetailsPage = () => {
  ...
  const renderParticipantsByStatus = (status, title, participants) => {
    ...
    return (
      <div className="mb-6">
        <h4 className="text-lg font-semibold mb-3 flex items-center space-x-2" style={{ color: 'var(--primary)' }}>
          <span>{eventService.formatStatus(status).icon}</span>
          <span>{title} ({statusParticipants.length})</span>
        </h4>
          ...
          {statusParticipants.map((participant) => (
            ...
            >
              <div 
                ...
                style={{ backgroundColor: eventService.formatStatus(participant.status).color }}
              >
              ...
              {user && user.role === 'admin' && (
                <div className="text-sm">
                  <span 
                    ...
                    style={{ 
                      backgroundColor: eventService.formatStatus(participant.status).color + '20',
                      color: eventService.formatStatus(participant.status).color
                    }}
                  >
  ...
  const isOrganizer = user && (user.id === event.organizer_id || user.id == event.organizer_id || parseInt(user.id) === parseInt(event.organizer_id));
          ...
          <Box component="main" sx={{ flexGrow: 1, p: 0 }}>
            ...
            <div className="container mx-auto max-w-5xl px-4 py-8">
              ...
              <div className="flex space-x-8 border-b border-gray-200 mb-8">
                <div 
                  className={`tab py-2 ${activeTab === 'overview' ? 'active' : ''}`}
                  onClick={() => setActiveTab('overview')}
                >
                  Overview
                </div>
                <div 
                  className={`tab py-2 ${activeTab === 'agenda' ? 'active' : ''}`}
                  onClick={() => setActiveTab('agenda')}
                >
                  Agenda
                </div>
                <div 
                  className={`tab py-2 ${activeTab === 'participants' ? 'active' : ''}`}
                ...
                <div className="tab-content">
                  <div className="grid md:grid-cols-3 gap-8">
                    <div className="md:col-span-2">
                      <h2 className="text-2xl font-bold mb-4" style={{ color: 'var(--primary)' }}>About This Event</h2>
                      ...
                      {event.tags && event.tags.length > 0 && (
                        <div className="mt-6">
                          <h3 className="text-lg font-semibold mb-3" style={{ color: 'var(--primary)' }}>Topics</h3>
                          <div className="flex flex-wrap gap-2">
                            {event.tags.map((tag, index) => (
                              <span 
                                key={index}
                                className="px-3 py-1 rounded-full text-sm font-medium"
                                style={{ backgroundColor: 'var(--light-bg)', color: 'var(--primary)' }}
                              >
                                {tag.name}
                              </span>
                            ))}
                    ...
                    <div className="space-y-6">
                      {/* Event Details Card */}
                      <div className="bg-white rounded-lg p-6 border" style={{ borderColor: 'var(--light-bg)' }}>
                        <h3 className="text-lg font-semibold mb-4" style={{ color: 'var(--primary)' }}>Event Details</h3>
                        <div className="space-y-3">
                          <div className="flex items-start space-x-3">
                            <svg className="h-5 w-5 mt-0.5" style={{ color: 'var(--secondary)' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 002 2z"></path>
                            </svg>
                            <div>
                              <p className="font-medium">Date & Time</p>
                              <p className="text-gray-600 text-sm">{formatDateTime(event.start_date, event.end_date)}</p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-start space-x-3">
                            ...
                            <div>
                              ...
                              <p className="text-gray-600 text-sm">{formatEventFormat(event.format)}</p>
                            </div>
                          </div>
                          ...
                          {event.organizer && (
                            <div className="flex items-start space-x-3">
                              ...
                              <div>
                                ...
                                <p className="text-gray-600 text-sm">{event.organizer.first_name} {event.organizer.last_name}</p>
                              </div>
                            </div>
                          )}
                        </div>
              ...
              {activeTab === 'agenda' && (
                <div className="tab-content">
                  <h2 className="text-2xl font-bold mb-6" style={{ color: 'var(--primary)' }}>Event Agenda</h2>
                  <div className="bg-white rounded-lg p-6 border" style={{ borderColor: 'var(--light-bg)' }}>
                    <p className="text-gray-600">Agenda details will be available soon.</p>
                  </div>
                </div>
              )}
                ...
                <div className="tab-content">
                  ...
                  {participantsLoading ? (
                    <div className="flex items-center justify-center py-12">
                      <div className="animate-spin rounded-full h-8 w-8 border-b-2" style={{ borderColor: 'var(--como)' }}></div>
                      <span className="ml-3 text-gray-600">Loading participants...</span>
                    </div>
                  ) : participants.length === 0 ? (
                    <div className="bg-white rounded-lg p-8 border text-center" style={{ borderColor: 'var(--light-bg)' }}>
                      <div className="text-gray-400 text-6xl mb-4">👥</div>
                      <h3 className="text-lg font-medium text-gray-900 mb-2">No participants yet</h3>
                      <p className="text-gray-600">Be the first to RSVP to this event!</p>
                    </div>
                  ) : (
                    <div>
                      {/* Confirmed Participants */}
                      {renderParticipantsByStatus('confirmed', 'Confirmed Attendees', participants)}
                      
                      {/* Registered Participants */}
                      {renderParticipantsByStatus('registered', 'Going', participants)}
                      
                      {/* Cancelled Participants (only show to organizers) */}
                      {isOrganizer && renderParticipantsByStatus('cancelled', 'Not Going', participants)}
                      
                      {/* Summary for organizers */}
                      {isOrganizer && (
                        <div className="mt-8 bg-orange-50 rounded-lg p-4 border border-orange-200">
                          <h4 className="font-semibold text-orange-800 mb-2">Organizer Summary</h4>
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                            <div>
                              <span className="text-orange-600">Total Responses:</span>
