...
const ApiService = {
  ...
  async getEvents(filters = {}) {
    try {
      ...
      return events.map(event => ({
        ...
        cover_image: event.cover_image || `https://source.unsplash.com/800x400/?event&sig=${event.id}`
      }));
  ...
  getMockEvents() {
    ...
    const events = [];
    const eventTypes = ['Conference', 'Workshop', 'Meetup', 'Hackathon', 'Webinar'];
    const locations = ['Berlin', 'San Francisco', 'Tokyo', 'London', 'New York'];
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      events.push({
        id: `past-${i}`,
        title: `${eventTypes[Math.floor(Math.random() * eventTypes.length)]} in ${locations[Math.floor(Math.random() * locations.length)]}`,
        start_date: startDate.toISO(),
        end_date: startDate.plus({ hours: Math.floor(Math.random() * 3) + 1 }).toISO(),
        description: 'Join us for this exciting event!',
        location: locations[Math.floor(Math.random() * locations.length)],
        cover_image: `https://source.unsplash.com/random/800x400?event&sig=${i}`,
        rsvp_status: rsvpStatuses[Math.floor(Math.random() * rsvpStatuses.length)],
    ...
    for (let i = 1; i <= 15; i++) {
      ...
      events.push({
        ...
        description: 'Join us for this exciting event!',
        location: locations[Math.floor(Math.random() * locations.length)],
        cover_image: `https://source.unsplash.com/random/800x400?event&sig=${i+10}`,
        rsvp_status: rsvpStatuses[Math.floor(Math.random() * rsvpStatuses.length)],
        tags: ['tech', 'innovation'],
        author: 'Event Team',
        format: Math.random() > 0.5 ? 'in_person' : 'virtual'
...
function IOEventCard({ event, onClick }) {
  ...
  return (
    <div className="io-card bg-white shadow-md cursor-pointer" onClick={() => onClick(event.id)}>
      <div className="io-card-image">
        <img src={event.cover_image} alt={event.title} />
      </div>
      <div className="io-card-content">
        <div className="flex items-center gap-2 mb-3">
          <span className="io-badge" style={{ backgroundColor: 'var(--light-bg)', color: 'var(--primary)' }}>
            {formatEventFormat(event.format)}
          </span>
          {isPast && (
            <span className="io-badge" style={{ backgroundColor: '#e5e7eb', color: '#6b7280' }}>
              Past
            </span>
          )}
        </div>
        ...
        <div className="io-info">
          ...
          <span>{DateUtils.formatDateRange(event.start_date, event.end_date)}</span>
        </div>
        ...
        <div className="io-tags">
          {event.tags && event.tags.map((tag, index) => (
            <span key={index} className="io-tag">#{tag}</span>
          ))}
        </div>
        
        <div className="io-action">
          <button className="io-button io-button-secondary">Details</button>
          <button className={`io-button io-button-primary ${getRsvpBadgeClass(event.rsvp_status)}`}>
...
function FilterBar({ filters, onFilterChange }) {
  ...
  const filterOptions = {
    tags: ['conference', 'workshop', 'social', 'meetup', 'hackathon', 'webinar', 'rip', 'murat', 'dogs'],
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        {Object.keys(localFilters).length > 0 && (
          <button 
            ...
            style={{ backgroundColor: 'var(--gray-nurse)', color: 'var(--como)' }}
          >
            Clear all
          </button>
        )}
      ...
      {showFilters && (
        <div className="bg-white p-6 rounded-xl shadow-md mb-6 fade-in">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              <h3 className="font-medium mb-3" style={{ color: 'var(--como)' }}>Tags</h3>
              <div className="flex flex-wrap gap-2">
                {filterOptions.tags.map(tag => (
                  <button 
                    ...
                    style={{
                      backgroundColor: localFilters.tag === tag ? 'var(--moss-green-darker)' : 'var(--moss-green)',
                      color: 'white'
                    }}
                    onClick={() => handleFilterChange('tag', localFilters.tag === tag ? '' : tag)}
                  >
                    {tag}
                  </button>
                ))}
            ...
            <div>
              <h3 className="font-medium mb-3" style={{ color: 'var(--como)' }}>Format</h3>
              <div className="flex flex-wrap gap-2">
                {filterOptions.formats.map(format => (
                  <button 
                    ...
                    style={{
                      backgroundColor: localFilters.format === format ? 'var(--moss-green-darker)' : 'var(--moss-green)',
                      color: 'white'
                    }}
                    onClick={() => handleFilterChange('format', localFilters.format === format ? '' : format)}
                  >
                    {format.replace('_', '-')}
                  </button>
                ))}
            ...
            <div>
              <h3 className="font-medium mb-3" style={{ color: 'var(--como)' }}>Author</h3>
              <input 
                type="text" 
...
function MonthView({ events, currentDate, onEventClick }) {
  ...
  return (
    <div className="month-view bg-white rounded-xl shadow-md overflow-hidden">
      <div className="grid grid-cols-7 gap-0 text-center py-4 text-white" style={{ backgroundColor: 'var(--como)' }}>
        <div>Sun</div>
        <div>Mon</div>
        <div>Tue</div>
        <div>Wed</div>
      ...
      <div className="grid grid-cols-7 gap-0">
        {days.map((day, index) => {
          ...
          return (
            <div 
              key={index} 
              className={`day p-2 ${!isCurrentMonth ? 'bg-gray-50 text-gray-400' : ''} ${isToday ? 'bg-blue-50' : ''}`}
              ...
              <div className="space-y-1">
                {dayEvents.slice(0, 3).map((event, idx) => (
                  <div 
                    ...
                    onMouseEnter={(e) => e.target.style.backgroundColor = 'var(--moss-green-dark)'}
                    onMouseLeave={(e) => e.target.style.backgroundColor = 'var(--moss-green)'}
                  >
  ...
  return (
    <div className="week-view bg-white rounded-xl shadow-md overflow-hidden">
      <div className="grid grid-cols-8 gap-0">
        <div className="text-white p-4 font-medium" style={{ backgroundColor: 'var(--como)' }}>Time</div>
        {weekDays.map((day, index) => (
          <div key={index} className="text-white p-4 text-center" style={{ backgroundColor: 'var(--como)' }}>
      ...
      <div className="max-h-96 overflow-y-auto">
        {Array.from({ length: 24 }, (_, hour) => (
          <div key={hour} className="grid grid-cols-8 gap-0 hour-slot">
            <div className="p-2 text-sm text-gray-500 border-r" style={{ borderColor: 'var(--gray-nurse)' }}>
            ...
            {weekDays.map((day, dayIndex) => {
              ...
              return (
                <div key={dayIndex} className="p-1 border-r border-b" style={{ borderColor: 'var(--gray-nurse)' }}>
    ...
    <div className="agenda-view space-y-6 pb-12">
      {Object.entries(eventsByDate).map(([date, dayEvents]) => (
        <div key={date} className="bg-white rounded-xl shadow-md overflow-hidden">
          <div className="text-white p-4" style={{ backgroundColor: 'var(--moss-green)' }}>
          ...
          <div className="p-6">
            <div className={getGridClass(dayEvents.length)}>
              {dayEvents.map(event => (
                <div key={event.id} className={getEventContainerClass(dayEvents.length)}>
...
function HeroSection() {
  return (
    <div className="hero-section text-white py-16" style={{ 
...
function EventsPage() {
  ...
  const renderCalendarView = () => {
    if (eventsLoading) {
      return (
        <div className="bg-white rounded-xl shadow-md p-8 text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 mx-auto mb-4" style={{ borderColor: 'var(--moss-green)' }}></div>
      ...
      <Box sx={{ display: 'flex', flexGrow: 1 }}>
        ...
        >
          ...
          >
            ...
            <div style={{ backgroundColor: 'var(--spring-wood)', minHeight: 'calc(100vh - 64px)' }}>
              <HeroSection />
                  ...
                  <div className="flex flex-col md:flex-row justify-between items-center mb-6">
                    <div className="flex items-center space-x-4 mb-4 md:mb-0">
                      ...
                      <h2 className="text-xl font-semibold text-white">
                        {viewMode === 'month' && currentDate.toFormat('LLLL yyyy')}
                        {viewMode === 'week' && `${currentDate.startOf('week').toFormat('LLL d')} - ${currentDate.endOf('week').toFormat('LLL d, yyyy')}`}
                        {viewMode === 'agenda' && 'Upcoming Events'}
                      </h2>
                    </div>
                    
                    <div className="flex flex-wrap gap-4">
                      <div className="p-1 rounded-lg flex" style={{ backgroundColor: 'rgba(134, 182, 151, 0.3)' }}>
                        <button 
                          className={`view-toggle-btn px-4 py-2 rounded-lg text-sm ${viewMode === 'month' ? 'active' : ''}`}
                          onClick={() => setViewMode('month')}
                        >
                          Month
                        </button>
                        <button 
                          className={`view-toggle-btn px-4 py-2 rounded-lg text-sm ${viewMode === 'week' ? 'active' : ''}`}
                          onClick={() => setViewMode('week')}
                        >
                          Week
                        </button>
                        <button 
                          className={`view-toggle-btn px-4 py-2 rounded-lg text-sm ${viewMode === 'agenda' ? 'active' : ''}`}
