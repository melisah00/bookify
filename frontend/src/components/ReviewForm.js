...
function ReviewForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!user || !user.id) {
      setError("Morate biti ulogovani da biste ostavili recenziju. Bićete preusmjereni na stranicu za prijavu.");
      setIsLoading(false);
    ...
    if (rating === 0) {
      setError("Molimo odaberite ocjenu (1-5).");
      return;
    }

    if (!bookId) {
      setError("ID knjige nije pronađen. Pokušajte ponovo.");
      return;
    }
  ...
  return (
    <>
      ...
      >
        <div // Ovo je bio originalni spoljni div forme
          style={{
